# ---------------------------------------------------------------------------------
# Dialog to perform flexible fitting through FitOpt algorithm.
#
...
class FitOpt_Dialog(ModelessDialog):
    ...
    def fillInUI(self, parent):
        ...
        # Advanced options
        # ostext = 'Opened PDBs:'
        # osh = Tkinter.Label(hf, font="Verdana 8 bold", fg="navy", text=ostext)
        # osh.grid(row=3, column=0, sticky='w')
        #
        # ostext2 = 'Push this button to update the opened PDBs:    '
        # osh2 = Tkinter.Label(hf, font="Verdana 7 italic", fg="black", text=ostext2)
        # osh2.grid(row=4, column=0, sticky='w')
        #
        # self.refresh_button = Tkinter.Button(hf, text="Refresh", font=self.arialBondF, height=1, command=self.show_pdbs)
        # self.refresh_button.grid(row=4, column=1, sticky='w')

        # Space
        # msg = Tkinter.Label(hf, anchor='w', justify='left')
        # msg.grid(row=5, column=0, sticky='ew')
        # row = row + 1
        # self.message_label = msg
        ...
        from chimera.widgets import MoleculeScrolledListBox, \
            ModelOptionMenu
        self.modelList = MoleculeScrolledListBox(self.pdbs, labelpos='nw', label_text="Select PDBs: ",
        ...
        self.modelList1 = MoleculeScrolledListBox(self.pdbs, labelpos='nw', label_text="Fix PDBs: ",
                                                 listbox_selectmode='extended', listbox_height=11)
        ...
        # Shows opened PDBs with correspondant checks
        #self.show_pdbs()
    ...
    def show_pdbs(self):
        ...
        self.mlist = [m for m in fit_object_models() if isinstance(m, Molecule)]
        self.mlist_states = dict([(m.name, 0) for m in self.mlist])
        self.mlist_buttons = {}
        # Initial row
        row = 7
        for pdb_state in sorted(self.mlist_states):
            # Check for fitting
            self.mlist_states[pdb_state] = Tkinter.IntVar()
            c = Tkinter.Checkbutton(self.pdbs, padx=0, pady=0, bd=0, variable=self.mlist_states[pdb_state])
            c.grid(row=row, column=0, sticky='w')
            # PDB's name
            oshR = Tkinter.Label(self.pdbs, font=self.arialF, text=" " * 4 + "  " + pdb_state)
            oshR.grid(row=row, column=1, sticky='w')
            # Show fitted button
            button = Tkinter.Button(self.pdbs, text="Show fitted", font=self.arialBondF, height=1, state="disabled",
                                    command=lambda pdb_state=pdb_state: self.show_fitted_molecule(pdb_state))
            button.grid(row=row, column=2, sticky='w')
            self.mlist_buttons[pdb_state] = button
            row += 1
